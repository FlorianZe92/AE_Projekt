...
void run_pattern(vector<float> x_start, vector<float> y_start, string target, int it, int de, int size){
    ...
    // Read/Write CSV //
    ////////////////////
    string insert_x ="x.csv";
    string insert_y ="y.csv";
...
int main(int argc, char** argv){
    ...
    int it = atoi(argv[3]);
    int de = 2;
    // In diese Vektoren werden die Eingangsdaten eingelesen.
    vector<float> x;
    vector<float> y;
    ...
    //Je nach gewünschter Startform, werden hierbei die csv Dateien eingelesen. (Aus Input Ordner)
    if (shape_start == dino){
